...
function getAverageIncidentAtCoords(lat, lng, monthOfYear){
  ...
  //console.log(": ["+nasaDirect[latIndex][lngIndex].toString()+"],");
...
function getTroughCollected(lat, lng, monthOfYear,apertureArea, troughInclination ){
  ...
  const totalCollectedEnergy = Number.parseFloat(Number(collectedEnergyPerMeter * apertureArea).toFixed(2));
...
function getSoltiguaCollected(lat, lng, monthOfYear,apertureArea, troughInclination){
 // console.log(lat);
  ...
  const incidentEnergy = getAverageIncidentAtCoords(lat, lng, monthOfYear);
  let efficencyRating = document.getElementById("troughEfficiency").value.replace(/[^\d.-]/g, '')/100;
...
function getDishCollected(lat, lng, monthOfYear,apertureArea){
 ...
 function getPVCollected(lat, lng, monthOfYear, panelInclination, apertureArea){
  ...
  //console.log(losses);
  url = "https://developer.nrel.gov/api/pvwatts/v6.json?api_key=DEMO_KEY&lat=40.5&lon=-75&system_capacity=6&azimuth=180&tilt=40&array_type=4&module_type=1&losses=10"
...
function getPVEfficiency(){
 ...
 // let snow = 0 %;
  ...
  let wiring = 0.98;
  let connections = .995;
  let lightInducedDegradation =.985;
  let nameplateRating =.99;
  let availability =.97;

  let losses = soiling*shading*mismatch*wiring*connections*lightInducedDegradation*nameplateRating*availability;
...
function getTroughEfficiency(lat, lng, monthOfYear, troughInclination){
  ...
  const param_solarWeightedReflectance =  document.getElementById("solarWeightedReflectance").value;  //0.88;
  ...
  const troughEfficiency = param_cosineEffect * param_shading * param_solarWeightedReflectance * param_cleanliness * param_IAM * param_endLosses * param_intercept * param_shieldingByBellows * param_glassTransmission * param_receiverAbsorptance * param_receiverThermalLosses;
...
function getPVEfficiencyOld(lat, lng, monthOfYear, panelInclination){
  ...
  const PLACEHOLDER_EFFICIENCY = .19; //assume 19 percent effiency
...
function getFixedAxisCosineEffect(lat, lng, monthOfYear, panelInclination){
  ...
  for (var i = 1; i <= daysInMonth[monthOfYear]; i++) {
    const daysAngleOfIncidence = getAvgSolarZenithForDay(lat, lng, i, monthOfYear, panelInclination );
...
function getDishEfficiency(dailyAvgIncident) {
  ...
  const param_cleanliness = .98; // was .99 changed to fix percentage
  ...
  const dishEfficiency = param_solarWeightedReflectance * param_cleanliness * para_DishGeometricDistortions * para_DishThermalDistortions * param_receiverStrutShadow * param_reflectorIntegrity * param_receiverConversionEfficiency;
